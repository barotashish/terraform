...
func (s *HTTPMirrorSource) PackageMeta(provider addrs.Provider, version Version, target Platform) (PackageMeta, error) {
	...
	statusCode, body, finalURLStr, err := s.get(endpointPath)
	defer func() {
		if body != nil {
			body.Close()
		}
	...
	case http.StatusUnauthorized, http.StatusForbidden:
		return PackageMeta{}, s.errUnauthorized(finalURLStr)
	default:
	...
	// If we got a final URL string then we'll parse it so we can use it
	// to resolve relative URLs inside the index file later.
	var finalURL *url.URL
	if finalURLStr != "" {
		finalURL, err = url.Parse(finalURLStr)
		if err != nil {
			// It would be weird to get here because this URL was generated
			// by the net/http client.
			return PackageMeta{}, s.errQueryFailed(provider, fmt.Errorf("final request URL %q is invalid: %s", finalURL, err))
		}
	}
...
func (s *HTTPMirrorSource) get(relativePath string) (statusCode int, body io.ReadCloser, finalURL string, error error) {
	...
	if err != nil {
		return 0, nil, "", fmt.Errorf("failed to determine request credentials: %s", err)
	}
	if creds != nil {
	...
	finalURL = resp.Request.RequestURI
...
func (s *HTTPMirrorSource) errUnauthorized(finalURL string) error {
	u, err := url.Parse(finalURL)
	if err != nil {
		// It would be weird for our final URL to be invalid, but we'll
		// tolerate it anyway to be robust, just returning less-helpful error.
		return fmt.Errorf("invalid credentials for %s", finalURL)
	}

	hostname, err := svchostFromURL(u)
	if err != nil {
...
func svchostFromURL(u *url.URL) (svchost.Hostname, error) {
	...
	// When "friendly hostnames" appear in Terraform-specific contexts we
